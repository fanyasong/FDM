...
FDM decode memory is strictly O(1) — fixed at 867 MB regardless of prompt length. Transformer KV cache grows linearly from 853 MB to 4,247 MB at N=8,192.
...
No single linear operator can simultaneously be norm-preserving (wave-like) AND selectively forgetting (particle-like). FDM resolves this by explicit separation.

Freeze-Scan Training
Standard joint training causes a gradient sink: scan parameters (14.9M) dominate gradient flow, starving the cache (122.1M). Solution:
...
We interpret h_t as a holographic plate: the complex hidden state encodes the entire temporal history via phase interference.
...
>90% of holographically recoverable information lives in layer 0. Consistent with AdS/CFT: information on the boundary, not the bulk.
...
📄 License
MIT License. Core technology subject to Chinese patent No. 2026104740169 (filed 2026-04-11), with priority from No. 2026104567714 (filed 2026-04-08).
